...
*DATA:  mtab_program_file LIKE textpool OCCURS 0 WITH HEADER LINE.
...
USING  f_filename.
  ...
  ELSE.            " Save file to application server
    OPEN DATASET f_filename FOR OUTPUT IN TEXT MODE ENCODING UTF-8.
    IF sy-subrc = 0.
      LOOP AT ftab_table.
...
ELSEIF rb_unix = 'X'.
    ...
    OPEN DATASET f_filename FOR INPUT MESSAGE lc_message IN TEXT MODE ENCODING UTF-8.
    IF sy-subrc = 0.
      DO.
...
CHANGING fstr_trdir
  ...
  LOOP AT ftab_program_file.
    ...
    WHEN OTHERS. " Actual contents of report, trdir, or text
      ...
      WHEN mc_text_short.
        ftab_program_texts-ENTRY = ftab_program_file-LINE.
